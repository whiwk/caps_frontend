...
const DataEdge = React.memo(({ element, onEdgeClick }) => {
  const handleEdgeClick = (e) => {
      e.stopPropagation();
      onEdgeClick(element);
  };

  const edgeClass = element.getData()?.className || '';
  const startTerminalClass = element.getData()?.startTerminalClass || '';
  const endTerminalClass = element.getData()?.endTerminalClass || '';

  console.log('DataEdge rendered with className:', edgeClass);
  console.log('Start Terminal Class:', startTerminalClass);
  console.log('End Terminal Class:', endTerminalClass);
  ...
  return (
      <DefaultEdge
          ...
          className={`pf-topology__edge__link ${edgeClass}`}
          startTerminalClass={`pf-topology__connector-circle pf-m-source ${startTerminalClass}`}
          endTerminalClass={`pf-topology__connector-circle ${endTerminalClass}`}
      />
  );
});
...
export const TopologyCustomEdgeDemo = () => {
        ...
        if (pod) {
          ...
          if (response.data && response.data.packets && Array.isArray(response.data.packets)) {
            ...
            if (requestIdRef.current === currentRequestId) {
              setProtocolStackData(parsedData[0] || {});
            }
          } else {
    ...
    const getEdgeClass = (id, userInfo) => {
      let edgeClass = '';
      switch (id) {
          case 'air-interface':
              edgeClass = (userInfo.du_matches === 12 && userInfo.ue_matches === 9) ? 'green' : 'red';
              break;
          case 'Open-Fronthaul-interface':
              edgeClass = (userInfo.du_matches === 12) ? 'green' : 'red';
              break;
          case 'F1-interface':
              edgeClass = (userInfo.cu_matches === 12 && userInfo.du_matches === 12) ? 'green' : 'red';
              break;
          case 'N2-interface':
          case 'N3-interface':
              edgeClass = (userInfo.cu_matches === 12) ? 'green' : 'red';
              break;
          default:
              edgeClass = 'red';
      }
      console.log(`Edge ID: ${id}, Class: ${edgeClass}, User Info:`, userInfo);
      return edgeClass;
    };
  
    const edges = useMemo(() => {
      console.log('Generating edges with userInfo:', userInfo);
      return [
          {
              id: 'air-interface',
              type: 'data-edge',
              ...
              data: {
                  className: getEdgeClass('air-interface', userInfo),
                  startTerminalClass: getEdgeClass('air-interface', userInfo),
                  endTerminalClass: getEdgeClass('air-interface', userInfo),
              }
          },
          {
              ...
              animationSpeed: EdgeAnimationSpeed.medium,
              data: {
                  className: getEdgeClass('Open-Fronthaul-interface', userInfo),
                  startTerminalClass: getEdgeClass('Open-Fronthaul-interface', userInfo),
                  endTerminalClass: getEdgeClass('Open-Fronthaul-interface', userInfo),
              }
          },
          {
              id: 'F1-interface',
              ...
              animationSpeed: EdgeAnimationSpeed.medium,
              data: {
                  className: getEdgeClass('F1-interface', userInfo),
                  startTerminalClass: getEdgeClass('F1-interface', userInfo),
                  endTerminalClass: getEdgeClass('F1-interface', userInfo),
              }
          },
          {
              id: 'N2-interface',
              ...
              animationSpeed: EdgeAnimationSpeed.medium,
              data: {
                  className: getEdgeClass('N2-interface', userInfo),
                  startTerminalClass: getEdgeClass('N2-interface', userInfo),
                  endTerminalClass: getEdgeClass('N2-interface', userInfo),
              }
          },
          {
              id: 'N3-interface',
              ...
              animationSpeed: EdgeAnimationSpeed.medium,
              data: {
                  className: getEdgeClass('N3-interface', userInfo),
                  startTerminalClass: getEdgeClass('N3-interface', userInfo),
                  endTerminalClass: getEdgeClass('N3-interface', userInfo),
              }
          },
      ];
  }, [userInfo]);
                    ...
                    switch (kind) {
                        ...
                        case ModelKind.edge:
                            return withSelection()(
                                (props) => (
                                    <DataEdge {...props} onSelect={() => handleEdgeClick(props.element)} edgeClass={props.element.getData().className} />
                                )
                            );
                        default:
                            return undefined;
    ...
    useEffect(() => {
        if (controller) {
            console.log('Updating controller with new edges:', edges);
